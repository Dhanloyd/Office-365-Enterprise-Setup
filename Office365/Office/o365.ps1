...
# Okunabilir ortam kontrolü / Read-only environment check
function Test-WritableDirectory {
    param([string]$path)
    try {
        $testFile = Join-Path $path ".writable_test_$(Get-Random).tmp"
...
$script:isReadOnlyEnvironment = -not (Test-WritableDirectory -path $PSScriptRoot)
$script:readOnlyWarningShown = $false

# Ana formu oluştur / Create main form
$form = New-Object System.Windows.Forms.Form
$initialFormTitle = $initialFormTitles[$script:currentLanguage]
if (-not $initialFormTitle) {
    $initialFormTitle = $initialFormTitles['en']
}
$form.Text = $initialFormTitle
...
# Minimize edilirse otomatik olarak geri yükle / Automatically restore if minimized
$form.Add_Resize({
...
# Formun Paint olayını kullanarak resmi yükle ve ayarla / Load and set image using form's Paint event
$form.Add_Paint({
    param($sender, $e)

    if ($logo) {
        $targetWidth = $form.ClientSize.Width / 2
        $targetHeight = $form.ClientSize.Height / 2
        $scaleWidth = $targetWidth / $logo.Width
        $scaleHeight = $targetHeight / $logo.Height
...
# Dili değiştiren fonksiyon / Function to change language
function Switch-Language {
    $script:currentLanguage = if ($script:currentLanguage -eq 'tr') { 'en' } else { 'tr' }

    $form.Text = $translations[$script:currentLanguage]['FormTitle']
    $licenseGroup.Text = $translations[$script:currentLanguage]['LicenseType']
    $archGroup.Text = $translations[$script:currentLanguage]['Architecture']
    $languageGroup.Text = $translations[$script:currentLanguage]['Language']
    $appsGroup.Text = $translations[$script:currentLanguage]['ApplicationsGroup']
    $installOptionsGroup.Text = $translations[$script:currentLanguage]['InstallOptions']
    $onlineRadioButton.Text = $translations[$script:currentLanguage]['Online']
    $offlineRadioButton.Text = $translations[$script:currentLanguage]['Offline']
    $archLabel.Text = $translations[$script:currentLanguage]['ArchitectureAutoMessage']
    $installButton.Text = $translations[$script:currentLanguage]['Install']
    $exitButton.Text = $translations[$script:currentLanguage]['Exit']
    $offlineInstallButton.Text = $translations[$script:currentLanguage]['OfflineInstall']

    $LinkLabel1.Text = $translations[$script:currentLanguage]['MyPage']
    $LinkLabel2.Text = $translations[$script:currentLanguage]['About']

    if ($statusLabel.Text -eq $translations['en']['Ready'] -or $statusLabel.Text -eq $translations['tr']['Ready']) {
        $statusLabel.Text = $translations[$script:currentLanguage]['Ready']
    } elseif ($statusLabel.Text -eq $translations['en']['Downloading'] -or $statusLabel.Text -eq $translations[$script:currentLanguage]['Downloading']) {
        $statusLabel.Text = $translations[$script:currentLanguage]['Downloading']
    } elseif ($statusLabel.Text -eq $translations['en']['Installing'] -or $statusLabel.Text -eq $translations[$script:currentLanguage]['Installing']) {
        $statusLabel.Text = $translations[$script:currentLanguage]['Installing']
    } else {
        if ($statusLabel.Text -match "Downloading") {
            $statusLabel.Text = $translations[$script:currentLanguage]['Downloading']
        } elseif ($statusLabel.Text -match "Installing") {
            $statusLabel.Text = $translations[$script:currentLanguage]['Installing']
        } else {
            $statusLabel.Text = $translations[$script:currentLanguage]['Ready']
        }
...
# translations sözlüğü / translations dictionary
$translations = @{
    'en' = @{
        'FormTitle' = 'Office 365 Enterprise Setup v2 | made by Abdullah ERTÜRK | www.TNCTR.com.'
        'Version' = 'Version'
        ...
        'Install' = 'Start Online Installing'
        'Exit' = 'Exit'
        'Check' = 'License Check'
        ...
        'RetailToVLConverter' = 'Retail to VL Converter'
        'RunScript' = 'Run Script'
        'Error' = 'Error'
        'Success' = 'Success'
        'CheckForChannelUpdates' = 'Get Latest Channel Information'
        'CheckingPublicOfficeChannels' = 'Checking public Office distribution channels for new updates'
        'About' = 'About'
        'InstallOptions' = 'Install Options'
        'Online' = 'Online'
        'Offline' = 'Offline'
        'TogglePanelOpen' = 'Open Side Panel'
        'TogglePanelClose' = 'Close Side Panel'
        'OfflineInstall' = 'Start Offline Installing'
        'AdminRightsRequired' = 'This application requires administrator privileges. Please run PowerShell as administrator.'
        'InfoMessage' = "`n`n`n`nThis application has been developed to install Office 365 Enterprise version on your computer and is only compatible with Windows 10/11 operating systems."
        'CheckFileNotFound' = 'Check.bat not found:'
        'LicenseLoadError' = 'An error occurred while loading the license - Exit code:'
        'OnlyOffice365Supported' = 'This application only supports Office 365 installation.'
        'VolumeLicenseError' = 'An error occurred while loading Volume License certificates.'
        'NoInternetConnection' = 'You do not have an internet connection. Please check your connection and try again.'
        'AboutMessage' = "This application was coded by Abdullah ERTÜRK."
        'AboutTitle' = 'Office 365 Enterprise Setup'
        'ReadOnlyEnvironmentMessage' = "When this application is run from a read-only environment, the ''Install Options'' and ''Office 365 Interface Language'' preferences and`nthe ''Start Online Installation'' button are disabled. You can only use the ''Start Offline Installation'' option."
        'VolumeLicenseLoading' = 'Loading Volume License certificates, please wait...'
        'VolumeLicenseSuccess' = 'Volume License certificates successfully loaded.'
        'ApplicationsGroup' = ' Office 365 Enterprise Applications'
        'ArchitectureAutoMessage' = "Architecture selection is automatic in both Offline and Online installation.`nDo not interfere."
        'SetupExeNotFound' = 'setup.exe not found:'
        'DownloadFailed' = 'Download failed. Exit code:'
        'OfficeConfiguring' = 'Configuring Office...'
        'InstallationSuccess' = "Office 365 Enterprise has been successfully installed."
        'InstallationFailed' = 'Installation appears to have failed. Exit code:'
        'ConfirmDownload' = 'Confirm Download'
        'ContinueInstallation' = 'Continue Installation'
        'DownloadCompleteQuestion' = 'Download completed. Start Office 365 Enterprise installation?'
        'MyPage' = 'Github'
        'UnsupportedOfficeVersion' = 'Unsupported Office version:'
        'OfficeRootNotFound' = 'Microsoft Office (root) directory not found. Volume license certificates could not be loaded.'
        'LicensesFolderNotFound' = 'Licenses16 directory not found:'
        'OfficeFolderNotFound' = 'Office16 directory not found:'
        'OsppNotFound' = 'ospp.vbs not found:'
        ...
        'OfflineFilesNotFound' = 'Office 365 offline installation files not found in the directory. Please download the files first or check the file path.'
        'OfflineFilesNotFoundTitle' = 'Files Not Found'
        'ErrorCode1603' = 'Installation failed with exit code 1603 (Fatal Error).`n`nPossible causes:`n• Insufficient administrator privileges`n• Antivirus software blocking the installation`n• Existing Office installation conflict`n• Insufficient disk space`n• Windows Installer service issue`n• Locked or in-use files`n`nRecommended solutions:`n1. Run the script as Administrator`n2. Temporarily disable antivirus software`n3. Uninstall any existing Office installations completely`n4. Free up disk space (at least 5 GB recommended)`n5. Restart Windows Installer service: services.msc`n6. Close all Office applications and try again`n7. Check Windows Event Viewer for detailed error logs'
    }
    'tr' = @{
        'FormTitle' = 'Office 365 Enterprise Kurulum v2 | made by Abdullah ERTÜRK | www.TNCTR.com.'
        'Version' = 'Sürüm'
        'LicenseType' = 'Lisans Türü'
        'Architecture' = 'İşlemci Mimarisi'
        'Branch' = 'Dal'
        'Channel' = 'Kanal'
        'Language' = 'Office 365 Arayüz Dili'
        'Applications' = 'Uygulamalar'
        'Download' = 'İndirmeyi Başlat'
        'Install' = 'Çevrimiçi Kurulumu Başlat'
        'Exit' = 'Çıkış'
        'Check' = 'Lisans Kontrol'
        'StartActivation' = 'Aktivasyon Başlat'
        'Ready' = 'Hazır'
        'Downloading' = 'İndiriliyor, lütfen bekleyin...'
        'DownloadComplete' = 'İndirme Tamamlandı'
        'Installing' = 'Yükleniyor, işlem uzun sürebilir, lütfen bekleyin...'
        'InstallComplete' = 'Kurulum başarıyla tamamlandı'
        'RetailToVLConverter' = 'Perakende to VL Çevirici'
        'RunScript' = 'Betiği Çalıştır'
        'Error' = 'Hata'
        'Success' = 'Başarılı'
        'CheckForChannelUpdates' = 'Güncel Kanal Bilgilerini Al'
        'CheckingPublicOfficeChannels' = 'Genel Office dağıtım kanalları için yeni güncellemeler kontrol ediliyor'
        'About' = 'Hakkında'
        'InstallOptions' = 'Kurulum Seçenekleri'
        'Online' = 'Çevrimiçi'
        'Offline' = 'Çevrimdışı'
        'TogglePanelOpen' = 'Yan Paneli Aç'
        'TogglePanelClose' = 'Yan Paneli Kapat'
        'OfflineInstall' = 'Çevrimdışı Kurulumu Başlat'
        'AdminRightsRequired' = 'Bu uygulama yönetici hakları gerektirir. Lütfen PowerShell''i yönetici olarak çalıştırın.'
        'InfoMessage' = "`n`n`n`nBu uygulama, bilgisayarınıza Office 365 Enterprise sürümünü yüklemek için geliştirilmiştir ve sadece Windows 10/11 işletim sistemleri ile uyumludur."
        'CheckFileNotFound' = 'Check.bat bulunamadı:'
        'LicenseLoadError' = 'lisansı yüklenirken hata oluştu - Çıkış kodu:'
        'OnlyOffice365Supported' = 'Bu uygulama sadece Office 365 kurulumunu desteker.'
        'VolumeLicenseError' = 'Volume Lisans sertifikaları yüklenirken hata oluştu.'
        'NoInternetConnection' = 'İnternet bağlantınız yok. Lütfen bağlantınızı kontrol edin ve tekrar deneyin.'
        'AboutMessage' = "Bu uygulama Abdullah ERTÜRK tarafından kodlanmıştır."
        'AboutTitle' = 'Office 365 Enterprise Setup'
        'ReadOnlyEnvironmentMessage' = "Bu uygulama okunabilir bir ortamdan çalıştırıldığında ''Kurulum Seçenekleri'' ile ''Office 365 Arayüz Dili'' tercihleri ve`n''Çevrimiçi Kurulumu Başlat'' butonu devre dışı kalır. Sadece ''Çevrimdışı Kurulumu Başlat'' seçeneğini kullanabilirsiniz."
        'VolumeLicenseLoading' = 'Volume Lisans sertifikaları yükleniyor, lütfen bekleyin...'
        'VolumeLicenseSuccess' = 'Volume Lisans sertifikaları başarıyla yüklendi.'
        'ApplicationsGroup' = ' Office 365 Enterprise Uygulamaları'
        'ArchitectureAutoMessage' = "Çevrimdışı ve Çevrimiçi kurulumda mimari seçimi otomatik yapılır.`nMüdahale etmeyin."
        'SetupExeNotFound' = 'setup.exe bulunamadı:'
        'DownloadFailed' = 'İndirme başarısız. Çıkış kodu:'
        'OfficeConfiguring' = 'Office yapılandırılıyor...'
        'InstallationSuccess' = "Office 365 Enterprise başarıyla kuruldu."
        'InstallationFailed' = 'Kurulum başarısız görünüyor. Çıkış kodu:'
        'ConfirmDownload' = 'İndirmeyi Onayla'
        'ContinueInstallation' = 'Kuruluma Devam Et'
        'DownloadCompleteQuestion' = 'İndirme tamamlandı. Office 365 Enterprise kurulumu başlatılsın mı?'
        'MyPage' = 'Github'
        'UnsupportedOfficeVersion' = 'Desteklenmeyen Office sürümü:'
        'OfficeRootNotFound' = 'Microsoft Office (root) dizini bulunamadı. Volume lisans sertifikaları yüklenemedi.'
        'LicensesFolderNotFound' = 'Licenses16 dizini bulunamadı:'
        'OfficeFolderNotFound' = 'Office16 dizini bulunamadı:'
        'OsppNotFound' = 'ospp.vbs bulunamadı:'
        'LicenseFileNotFound' = 'lisans dosyası bulunamadı:'
        'ConfirmInstallation' = 'Kurulumu Onayla'
        'ErrorCode1603' = 'Kurulum 1603 çıkış kodu ile başarısız oldu (Ölümcül Hata).`n`nOlası nedenler:`n• Yetersiz yönetici ayrıcalıkları`n• Antivirüs yazılımı kurulumu engelliyor`n• Mevcut Office kurulumu çakışması`n• Yetersiz disk alanı`n• Windows Installer servisi sorunu`n• Kilitli veya kullanımda olan dosyalar`n`nÖnerilen çözümler:`n1. Scripti Yönetici olarak çalıştırın`n2. Antivirüs yazılımını geçici olarak devre dışı bırakın`n3. Mevcut Office kurulumlarını tamamen kaldırın`n4. Disk alanı açın (en az 5 GB önerilir)`n5. Windows Installer servisini yeniden başlatın: services.msc`n6. Tüm Office uygulamalarını kapatın ve tekrar deneyin`n7. Detaylı hata kayıtları için Windows Olay Görüntüleyiciyi kontrol edin'
        'OfficeVersion' = 'Office Sürümü:'
        'ArchitectureLabel' = 'Mimari:'
        'LicenseTypeLabel' = 'Lisans Türü:'
        'SelectedApplications' = 'Seçilen Uygulamalar:'
        'AdditionalProducts' = 'Ek Ürünler:'
        'OfflineFilesNotFound' = 'Dizinde Office 365 çevrimdışı kurulum dosyaları bulunamadı. Lütfen önce dosyaları indirin veya dosya yolunu kontrol edin.'
        'OfflineFilesNotFoundTitle' = 'Dosyalar Bulunamadı'
    }
}

# Bilgilendirme mesajı Label (koyu kırmızı, bold, kutu görünümü yok) / Information message Label (dark red, bold, no box appearance)
$richTextBox = New-Object System.Windows.Forms.Label
$richTextBox.Location = New-Object System.Drawing.Point(400, 30)
$richTextBox.Size = New-Object System.Drawing.Size(380, 130)
$richTextBox.AutoSize = $false
$richTextBox.Text = $translations[$script:currentLanguage]['InfoMessage']
$richTextBox.ForeColor = [System.Drawing.Color]::DarkBlue
$richTextBox.Font = New-Object System.Drawing.Font("Times New Roman", 12)
$richTextBox.BackColor = [System.Drawing.Color]::Transparent

# Sadece Office 365 / Only Office 365
$versions = @(
...
foreach ($version in $versions) {
    ...
    $radio.Size = New-Object System.Drawing.Size(200, 20)
    $radio.Text = $version.Text
    $radio.Tag = $version.Value
    $versionRadios[$version.Value] = $radio
    $y += 25
}
$versionRadios["office365"].Checked = $true

# Lisans Türü seçimi / License Type selection
$licenseGroup = New-Object System.Windows.Forms.GroupBox
$licenseGroup.Location = New-Object System.Drawing.Point(540, 340)
$licenseGroup.Size = New-Object System.Drawing.Size(240, 50)
$licenseGroup.Text = $translations[$script:currentLanguage]['LicenseType']

$rbRetail = New-Object System.Windows.Forms.RadioButton
$rbRetail.Location = New-Object System.Drawing.Point(10, 20)
$rbRetail.Size = New-Object System.Drawing.Size(100, 20)
$rbRetail.Text = "Retail"

$rbVolume = New-Object System.Windows.Forms.RadioButton
$rbVolume.Location = New-Object System.Drawing.Point(120, 20)
$rbVolume.Size = New-Object System.Drawing.Size(100, 20)
$rbVolume.Text = "Volume"
$rbVolume.Checked = $true

$licenseGroup.Controls.AddRange(@($rbRetail, $rbVolume))

$cmbChannel = New-Object System.Windows.Forms.ComboBox
$cmbChannel.Location = New-Object System.Drawing.Point(10, 20)
$cmbChannel.Size = New-Object System.Drawing.Size(200, 20)
$cmbChannel.DropDownStyle = "DropDownList"

# Dil seçimi / Language selection
$languageGroup = New-Object System.Windows.Forms.GroupBox
$languageGroup.Location = New-Object System.Drawing.Point(540, 285)
$languageGroup.Size = New-Object System.Drawing.Size(240, 50)
$languageGroup.Text = $translations[$script:currentLanguage]['Language']

$cmbLanguage = New-Object System.Windows.Forms.ComboBox
$cmbLanguage.Location = New-Object System.Drawing.Point(10, 20)
$cmbLanguage.Size = New-Object System.Drawing.Size(200, 20)
$cmbLanguage.DropDownStyle = "DropDownList"
$cmbLanguage.DropDownHeight = 200
$languageGroup.Controls.Add($cmbLanguage)
...
# Applications selection / Uygulamalar seçimi
$appsGroup = New-Object System.Windows.Forms.GroupBox
$appsGroup.Location = New-Object System.Drawing.Point(20, 390)
$appsGroup.Size = New-Object System.Drawing.Size(760, 80)
$appsGroup.Text = $translations[$script:currentLanguage]['ApplicationsGroup']
$form.Controls.Add($appsGroup)

$script:allApps = @(
 "Word", "Excel", "PowerPoint", "Outlook", "OutlookNew",
 "OneNote", "Publisher", "Access"
)

# Visio ve Project ayrı ürünler olarak eklenecek (checkbox listesinde gösterilecek) / Visio and Project will be added as separate products (will be shown in checkbox list)
$script:additionalProducts = @(
 "Visio", "Project"
)

# Tüm ürünleri tek listede birleştir (her satırda 5 olacak şekilde) / Combine all products in a single list (5 per row)
$script:allProducts = $script:allApps + $script:additionalProducts
...
# Tüm ürünleri tek döngüde işle / Process all products in a single loop
for ($i = 0; $i -lt $script:allProducts.Count; $i++) {
    $row = [Math]::Floor($i / $colCount)
    $col = $i % $colCount
    
    $checkbox = New-Object System.Windows.Forms.CheckBox
    $productName = $script:allProducts[$i]
    
    # Görünen ismi belirle / Determine display name
    $displayName = $productName
    if ($displayName -eq "OutlookNew") {
        $displayName = "Outlook (New)"
    } elseif ($displayName -eq "Outlook") {
        $displayName = "Outlook (Classic)"
    } elseif ($displayName -eq "Visio") {
        $displayName = "Visio Pro"
    } elseif ($displayName -eq "Project") {
        $displayName = "Project Pro"
    }
    
    $checkbox.Text = $displayName
    $checkbox.Location = New-Object System.Drawing.Point(($col * ($checkboxWidth + $margin) + $margin), ($row * $checkboxHeight + 20))
    $checkbox.Size = New-Object System.Drawing.Size($checkboxWidth, $checkboxHeight)
    $checkbox.Checked = $false
    if ($productName -in @("Word", "Excel", "PowerPoint")) {
        $checkbox.Checked = $true
    }
    $checkbox.Visible = $true
    
    # Visio Pro ve Project Pro için özel renklendirme / Special coloring for Visio Pro and Project Pro
    if ($productName -eq "Visio") {
        $checkbox.ForeColor = [System.Drawing.Color]::FromArgb(0, 120, 215)  # Mavi / Blue
        $checkbox.Font = New-Object System.Drawing.Font($checkbox.Font.FontFamily, $checkbox.Font.Size, [System.Drawing.FontStyle]::Bold)
    } elseif ($productName -eq "Project") {
        $checkbox.ForeColor = [System.Drawing.Color]::FromArgb(237, 125, 49)  # Turuncu / Orange
        $checkbox.Font = New-Object System.Drawing.Font($checkbox.Font.FontFamily, $checkbox.Font.Size, [System.Drawing.FontStyle]::Bold)
    }
    
    # allApps'te mi yoksa additionalProducts'te mi kontrol et / Check if in allApps or additionalProducts
    if ($productName -in $script:allApps) {
        $appCheckboxes[$productName] = $checkbox
    } else {
        $additionalCheckboxes[$productName] = $checkbox
    }
    
    $appsGroup.Controls.Add($checkbox)
}
...
$progressBar.ForeColor = [System.Drawing.Color]::FromArgb(0, 120, 215)  # Mavi - daha belirgin / Blue - more prominent
...
# Progress bar'ı form gösterildikten sonra hazırla / Prepare progress bar after form is shown
$form.Add_Shown({
    $progressBar.Style = [System.Windows.Forms.ProgressBarStyle]::Continuous
    $progressBar.Value = 0
    $progressBar.Visible = $true
...
$statusLabel.Location = New-Object System.Drawing.Point(20, 568)
...
$statusLabel.Add_MouseEnter({
    $statusLabel.BackColor = [System.Drawing.Color]::FromArgb(230, 230, 250)
})
$statusLabel.Add_MouseLeave({
    $statusLabel.BackColor = [System.Drawing.Color]::White
})

function AddButtonEffects($button) {
    # BackColor'ı sakla / Save BackColor
    $originalBackColor = $button.BackColor
    
    # FlatStyle = Flat kullan - BackColor bu şekilde çalışır / Use FlatStyle = Flat - BackColor works this way
    $button.FlatStyle = [System.Windows.Forms.FlatStyle]::Flat
    $button.FlatAppearance.BorderSize = 2
    
    # Her butonun rengine göre border rengi ayarla (biraz daha koyu ton) / Set border color according to each button's color (slightly darker tone)
    $r = $originalBackColor.R
    $g = $originalBackColor.G
    $b = $originalBackColor.B
    $borderColor = [System.Drawing.Color]::FromArgb([Math]::Max(0, $r - 40), [Math]::Max(0, $g - 40), [Math]::Max(0, $b - 40))
    $button.FlatAppearance.BorderColor = $borderColor
    
    # BackColor'ı tekrar ayarla (FlatStyle değişikliği sonrası) / Set BackColor again (after FlatStyle change)
    $button.BackColor = $originalBackColor
    $button.UseVisualStyleBackColor = $false
    
    # Her butonun rengine göre hover ve click efektleri (daha açık tonlar) / Hover and click effects according to each button's color (lighter tones)
    $hoverColor = [System.Drawing.Color]::FromArgb([Math]::Min(255, $r + 30), [Math]::Min(255, $g + 30), [Math]::Min(255, $b + 30))
    $clickColor = [System.Drawing.Color]::FromArgb([Math]::Max(0, $r - 20), [Math]::Max(0, $g - 20), [Math]::Max(0, $b - 20))
    $button.FlatAppearance.MouseOverBackColor = $hoverColor
    $button.FlatAppearance.MouseDownBackColor = $clickColor
}

# Kurulum Seçenekleri / Install Options
$installOptionsGroup = New-Object System.Windows.Forms.GroupBox
$installOptionsGroup.Location = New-Object System.Drawing.Point(20, 476)
$installOptionsGroup.Size = New-Object System.Drawing.Size(240, 60)
$installOptionsGroup.Text = $translations[$script:currentLanguage]['InstallOptions']
$form.Controls.Add($installOptionsGroup)

$onlineRadioButton = New-Object System.Windows.Forms.RadioButton
$onlineRadioButton.Location = New-Object System.Drawing.Point(10, 20)
$onlineRadioButton.Size = New-Object System.Drawing.Size(100, 20)
$onlineRadioButton.Text = $translations[$script:currentLanguage]['Online']
$installOptionsGroup.Controls.Add($onlineRadioButton)

$offlineRadioButton = New-Object System.Windows.Forms.RadioButton
$offlineRadioButton.Location = New-Object System.Drawing.Point(120, 20)
$offlineRadioButton.Size = New-Object System.Drawing.Size(100, 20)
$offlineRadioButton.Text = $translations[$script:currentLanguage]['Offline']
$offlineRadioButton.Checked = $true
$installOptionsGroup.Controls.Add($offlineRadioButton)

# Mimari seçimi / Architecture selection
$archGroup = New-Object System.Windows.Forms.GroupBox
$archGroup.Location = New-Object System.Drawing.Point(290, 340)
$archGroup.Size = New-Object System.Drawing.Size(240, 50)
$archGroup.Text = $translations[$script:currentLanguage]['Architecture']

$rbX64 = New-Object System.Windows.Forms.RadioButton
$rbX64.Location = New-Object System.Drawing.Point(10, 20)
$rbX64.Size = New-Object System.Drawing.Size(100, 20)
$rbX64.Text = "64 bit (x64)"

$rbX86 = New-Object System.Windows.Forms.RadioButton
$rbX86.Location = New-Object System.Drawing.Point(120, 20)
$rbX86.Size = New-Object System.Drawing.Size(100, 20)
$rbX86.Text = "32 bit (x86)"
...
$archLabel.Location = New-Object System.Drawing.Point(312, 300)
$archLabel.Size = New-Object System.Drawing.Size(220, 40)
$archLabel.Text = $translations[$script:currentLanguage]['ArchitectureAutoMessage']
$archLabel.TextAlign = [System.Drawing.ContentAlignment]::MiddleCenter

$archGroup.Controls.Add($archLabel)
$archGroup.Controls.AddRange(@($rbX64, $rbX86))

$offlineRadioButton.Add_CheckedChanged({
    if ($offlineRadioButton.Checked) {
        ...
        # Çevrimdışı seçildiğinde butonları ayarla / Set buttons when Offline is selected
        $offlineInstallButton.Enabled = $true
        $installButton.Enabled = $false
    }
})

$onlineRadioButton.Add_CheckedChanged({
    if ($onlineRadioButton.Checked) {
        ...
        # Çevrimiçi seçildiğinde butonları ayarla / Set buttons when Online is selected
        $installButton.Enabled = $true
        $offlineInstallButton.Enabled = $false
    }
})

# Offline Install button - Mavi renk / Offline Install button - Blue color
$offlineInstallButton = New-Object System.Windows.Forms.Button
$offlineInstallButton.Location = New-Object System.Drawing.Point(270, 480)
$offlineInstallButton.Size = New-Object System.Drawing.Size(200, 55)
$offlineInstallButton.TextAlign = [System.Drawing.ContentAlignment]::MiddleCenter
$offlineInstallButton.Text = $translations[$script:currentLanguage]['OfflineInstall']
$offlineInstallButton.UseVisualStyleBackColor = $false
$offlineInstallButton.BackColor = [System.Drawing.Color]::FromArgb(0, 120, 215)  
$offlineInstallButton.ForeColor = [System.Drawing.Color]::White
$offlineInstallButton.Font = New-Object System.Drawing.Font("Microsoft Sans Serif", 10, [System.Drawing.FontStyle]::Bold)
$offlineInstallButton.Visible = $true
AddButtonEffects $offlineInstallButton
$form.Controls.Add($offlineInstallButton)

# Install button (online + download/kurulum) - Yeşil renk / Install button (online + download/install) - Green color
$installButton = New-Object System.Windows.Forms.Button
$installButton.Location = New-Object System.Drawing.Point(485, 480)
$installButton.Size = New-Object System.Drawing.Size(200, 55)
$installButton.TextAlign = [System.Drawing.ContentAlignment]::MiddleCenter
$installButton.Text = $translations[$script:currentLanguage]['Install']
$installButton.UseVisualStyleBackColor = $false
$installButton.BackColor = [System.Drawing.Color]::FromArgb(16, 124, 16)  
$installButton.ForeColor = [System.Drawing.Color]::White
$installButton.Font = New-Object System.Drawing.Font("Microsoft Sans Serif", 10, [System.Drawing.FontStyle]::Bold)
AddButtonEffects $installButton
$form.Controls.Add($installButton)

# Başlangıç durumunu ayarla (offlineRadioButton varsayılan olarak seçili) / Set initial state (offlineRadioButton selected by default)
if ($offlineRadioButton.Checked) {
    $offlineInstallButton.Enabled = $true
    $installButton.Enabled = $false
} else {
...
# Exit butonu - Kırmızı renk / Exit button - Red color
$exitButton = New-Object System.Windows.Forms.Button
$exitButton.Location = New-Object System.Drawing.Point(703, 600)
$exitButton.Size = New-Object System.Drawing.Size(77, 30)
...
# Lisans Kontrol butonu - Turuncu renk / License Check button - Orange color
$checkButton = New-Object System.Windows.Forms.Button
$checkButton.Location = New-Object System.Drawing.Point(703, 480)
$checkButton.Size = New-Object System.Drawing.Size(77, 55)
$checkButton.TextAlign = [System.Drawing.ContentAlignment]::MiddleCenter
$checkButton.Text = $translations[$script:currentLanguage]['Check']
$checkButton.UseVisualStyleBackColor = $false
$checkButton.BackColor = [System.Drawing.Color]::FromArgb(255, 165, 0) 
$checkButton.ForeColor = [System.Drawing.Color]::White
$checkButton.Font = New-Object System.Drawing.Font("Microsoft Sans Serif", 9, [System.Drawing.FontStyle]::Bold)
AddButtonEffects $checkButton
...
$checkButton.Add_Click({
    ...
    $checkFilePath = "$scriptDir\Check.bat"
    if (Test-Path $checkFilePath) {
        Start-Process $checkFilePath
    } else {
        [System.Windows.Forms.MessageBox]::Show("$($translations[$script:currentLanguage]['CheckFileNotFound']) $checkFilePath", $translations[$script:currentLanguage]['Error'], [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
    }
})

# OfficeClientEdition / Office Client Edition
function Get-OfficeClientEdition {
    if ($rbX64.Checked) {
        return "64"
    } elseif ($rbX86.Checked) {
        return "32"
    }
}

# Yalnızca Office 365 için ProductId / ProductId for Office 365 only
function Get-ProductId {
    ...
    switch ($version) {
        "office365" {
            # Hem Retail hem Volume lisanslama bu ürün kimliğiyle kuruluyor
            return "O365ProPlusRetail"
        }
        default { throw "$($translations[$script:currentLanguage]['UnsupportedOfficeVersion']) $version" }
    }
}

function Get-OfficeRootPath {
    param (
        [string]$architecture
    )
    ...
    if ($architecture -eq "32") {
        if ($programFilesX86) {
            $candidatePaths += (Join-Path $programFilesX86 "Microsoft Office\root")
        }
        if ($programFiles) {
            $candidatePaths += (Join-Path $programFiles "Microsoft Office\root")
        }
    } else {
        if ($programFiles) {
            $candidatePaths += (Join-Path $programFiles "Microsoft Office\root")
...
function Install-VolumeLicenseCertificates {
    param (
        [string]$architecture,
        [string[]]$additionalProducts = @()
    )

    $officeRoot = Get-OfficeRootPath -architecture $architecture
    if (-not $officeRoot) {
        throw $translations[$script:currentLanguage]['OfficeRootNotFound']
    }

    $licensesFolder = Join-Path $officeRoot "Licenses16"
    if (-not (Test-Path $licensesFolder)) {
        throw "$($translations[$script:currentLanguage]['LicensesFolderNotFound']) $licensesFolder"
    }

    $officeFolder = Join-Path $officeRoot "Office16"
    if (-not (Test-Path $officeFolder)) {
        throw "$($translations[$script:currentLanguage]['OfficeFolderNotFound']) $officeFolder"
    }

    $osppPath = Join-Path $officeFolder "ospp.vbs"
    if (-not (Test-Path $osppPath)) {
        throw "$($translations[$script:currentLanguage]['OsppNotFound']) $osppPath"
    }
    ...
    function Install-LicenseFiles {
        ...
        $licenseFiles = Get-ChildItem -Path $licensesFolder -Filter $pattern -ErrorAction SilentlyContinue
        if (-not $licenseFiles -or $licenseFiles.Count -eq 0) {
            throw "$pattern ($description) $($translations[$script:currentLanguage]['LicenseFileNotFound']) $licensesFolder"
        }

        foreach ($licenseFile in $licenseFiles) {
        $arguments = @("//nologo", "`"$osppPath`"", "/inslic:`"$($licenseFile.FullName)`"")
        $process = Start-Process -FilePath "cscript.exe" -ArgumentList $arguments -PassThru -WindowStyle Hidden
        while (-not $process.HasExited) {
            Start-Sleep -Milliseconds 200
            [System.Windows.Forms.Application]::DoEvents()
        }
            if ($process.ExitCode -ne 0) {
                throw "$($licenseFile.Name) $($translations[$script:currentLanguage]['LicenseLoadError']) $($process.ExitCode)"
            }
        }
    }

    # Office 365 için Mondo lisanslarını yükle
    Install-LicenseFiles -pattern "MondoVL_*.xrm-ms" -description "Office 365"

    if ($additionalProducts -contains "Project") {
        Install-LicenseFiles -pattern "ProjectProVL*.xrm-ms" -description "Project Pro"
    }

    if ($additionalProducts -contains "Visio") {
        Install-LicenseFiles -pattern "VisioProVL*.xrm-ms" -description "Visio Pro"
    }
}

# Download/Install path – sadece Office365 klasör yapısı / Download/Install path – Office365 folder structure only
function Get-DownloadPath {
    ...
    if ($version -ne "office365") {
        throw $translations[$script:currentLanguage]['OnlyOffice365Supported']
    }

    return Join-Path $PSScriptRoot ("Office365\{0}\{1}" -f $architecture, $language)
}
...
# XML – DOWNLOAD / XML – İNDİRME
function Create-ConfigurationXmlDownload {
    param (
        [string]$basePath,
        [string]$productId,
        [string]$architecture,
    ...
    $clientEdition = ($architecture -replace '[^\d]', '')
    if ([string]::IsNullOrWhiteSpace($channel)) {
        $effectiveChannel = "MonthlyEnterprise"
    } else {
        $effectiveChannel = $channel
    }

    # OneDrive for Business ve Skype for Business'ı her zaman exclude et
    # Office 365 App ID'leri: OneDrive, Groove (OneDrive eski adı), Lync (Skype for Business eski adı)
    $alwaysExcludeApps = @("OneDrive", "Groove", "Lync")
    $allExcludeApps = @()
    if ($excludeApps -and $excludeApps.Count -gt 0) {
        $allExcludeApps = $excludeApps
    }
    # Always exclude apps'i ekle (eğer zaten yoksa)
    foreach ($app in $alwaysExcludeApps) {
        if ($app -notin $allExcludeApps) {
            $allExcludeApps += $app
        }
    }
    
    $excludeFragment = ""
    if ($allExcludeApps -and $allExcludeApps.Count -gt 0) {
        $excludeFragment = ($allExcludeApps | ForEach-Object {
            "      <ExcludeApp ID='$($_)' />"
        }) -join "`n"
    }

    $configPath = Join-Path ([System.IO.Path]::GetTempPath()) "configuration_download.xml"

    # Visio ve Project için ek Product'lar oluştur
    $additionalProductsFragment = ""
    if ($additionalProducts -and $additionalProducts.Count -gt 0) {
        foreach ($product in $additionalProducts) {
            $additionalProductId = ""
            if ($product -eq "Visio") {
                $additionalProductId = "VisioProRetail"
            } elseif ($product -eq "Project") {
                $additionalProductId = "ProjectProRetail"
            }
            if ($additionalProductId -ne "") {
                $additionalProductsFragment += "    <Product ID=`"$additionalProductId`">`n"
                $additionalProductsFragment += "      <Language ID=`"$language`" />`n"
                $additionalProductsFragment += "    </Product>`n"
            }
        }
    }
    ...
    $configXml = @"
<Configuration>
  <Add SourcePath="$basePath"
       OfficeClientEdition="$clientEdition"
...
$additionalProductsFragment
  ...
  <AppSettings>
    <Setup Name="Company" Value="Office 365 Enterprise" />
  </AppSettings>
</Configuration>
"@
...
# XML – ONLINE / XML – ÇEVRİMİÇİ
function Create-ConfigurationXmlOnline {
    param (
        [string]$productId,
        [string]$architecture,
        [string]$channel,
    ...
    $configPath = Join-Path ([System.IO.Path]::GetTempPath()) "configuration_online.xml"
    ...
    $configXml = @"
<Configuration>
  <Add OfficeClientEdition="$clientEdition"
       Channel="$effectiveChannel">
    <Product ID="$productId">
      <Language ID="$language" />
$excludeFragment
...
# XML – OFFLINE / XML – ÇEVRİMDIŞI
function Create-ConfigurationXmlOffline {
    param (
        [string]$basePath,
        [string]$productId,
        [string]$architecture,
    ...
    $configPath = Join-Path ([System.IO.Path]::GetTempPath()) "configuration_offline.xml"
...
# Office kurulu mu? (WINWORD.EXE testi) / Is Office installed? (WINWORD.EXE test)
...
# Butonları ve ilgili grupları kilitle/aç / Lock/unlock buttons and related groups
function Lock-Buttons {
    param([bool]$lock)

    # Ana işlem butonları
    $offlineInstallButton.Enabled = -not $lock
    $installButton.Enabled        = -not $lock
    $exitButton.Enabled           = -not $lock
    $checkButton.Enabled          = -not $lock

    # Kullanıcının kurulum esnasında değiştirmesini istemediğimiz alanlar
    if ($licenseGroup)         { $licenseGroup.Enabled         = -not $lock }   # Lisans Türü
    if ($languageGroup)        { $languageGroup.Enabled        = -not $lock }   # Office 365 Arayüz Dili
    if ($archGroup)            { $archGroup.Enabled            = -not $lock }   # İşlemci Mimarisi
    if ($appsGroup)            { $appsGroup.Enabled            = -not $lock }   # Office 365 Enterprise Uygulamaları
    if ($installOptionsGroup)  { $installOptionsGroup.Enabled  = -not $lock }   # Kurulum Seçenekleri
}

# Progress bar animasyonu için timer / Timer for progress bar animation
$script:progressTimer = $null
$script:progressValue = 0
$script:isProgressAnimating = $false

function Start-ProgressAnimation {
    # Timer ile manuel Marquee animasyonu
    $script:isProgressAnimating = $true
    $script:progressValue = 0
    $progressBar.Style = [System.Windows.Forms.ProgressBarStyle]::Continuous
    $progressBar.Minimum = 0
    $progressBar.Maximum = 100
    ...
    $script:progressTimer.Interval = 50  # 50ms'de bir güncelle
    $script:progressTimer.Add_Tick({
        if ($script:isProgressAnimating) {
            $script:progressValue += 2
            if ($script:progressValue -gt 100) {
                $script:progressValue = 0
            }
...
function Show-DetailedErrorMessage {
    param(
        [int]$ExitCode
    )
    
    # Özel hata kodları için detaylı mesajlar göster
    if ($ExitCode -eq 1603) {
        [System.Windows.Forms.MessageBox]::Show(
            $translations[$script:currentLanguage]['ErrorCode1603'],
            $translations[$script:currentLanguage]['Error'],
            [System.Windows.Forms.MessageBoxButtons]::OK,
            [System.Windows.Forms.MessageBoxIcon]::Error
        )
    } else {
        # Diğer hata kodları için genel mesaj
        [System.Windows.Forms.MessageBox]::Show(
            "$($translations[$script:currentLanguage]['InstallationFailed']) $ExitCode",
            $translations[$script:currentLanguage]['Error'],
            [System.Windows.Forms.MessageBoxButtons]::OK,
            [System.Windows.Forms.MessageBoxIcon]::Error
        )
...
# Çevrimdışı butonu / Offline button 
$offlineInstallButton.Add_Click({
    $selectedVersion = "office365"
    $architecture = Get-OfficeClientEdition
    $licenseType = if ($rbRetail.Checked) { "Retail" } else { "Volume" }
    $channel = $cmbChannel.SelectedItem
    $selectedApps = $appCheckboxes.GetEnumerator() | Where-Object { $_.Value.Checked } | Select-Object -ExpandProperty Key
    $selectedAdditionalProducts = $additionalCheckboxes.GetEnumerator() | Where-Object { $_.Value.Checked } | Select-Object -ExpandProperty Key

    # Outlook ve OutlookNew için özel mantık: biri seçilirse diğeri exclude edilmeli
    $excludeApps = $script:allApps | Where-Object { $_ -notin $selectedApps }
    if ("Outlook" -in $selectedApps -and "OutlookNew" -notin $selectedApps) {
        # Outlook seçilmiş, OutlookNew exclude edilmeli
        if ("OutlookNew" -notin $excludeApps) {
            $excludeApps += "OutlookNew"
        }
    } elseif ("OutlookNew" -in $selectedApps -and "Outlook" -notin $selectedApps) {
        # OutlookNew seçilmiş, Outlook exclude edilmeli
        if ("Outlook" -notin $excludeApps) {
            $excludeApps += "Outlook"
        }
    }

    $confirmationMessage = "$($translations[$script:currentLanguage]['OfficeVersion']) $selectedVersion`n$($translations[$script:currentLanguage]['ArchitectureLabel']) $architecture`n$($translations[$script:currentLanguage]['LicenseTypeLabel']) $licenseType`n$($translations[$script:currentLanguage]['SelectedApplications']) $($selectedApps -join ', ')"
    if ($selectedAdditionalProducts.Count -gt 0) {
        $confirmationMessage += "`n$($translations[$script:currentLanguage]['AdditionalProducts']) $($selectedAdditionalProducts -join ', ')"
    }
    $userResponse = [System.Windows.Forms.MessageBox]::Show($confirmationMessage, $translations[$script:currentLanguage]['ConfirmInstallation'], [System.Windows.Forms.MessageBoxButtons]::OKCancel, [System.Windows.Forms.MessageBoxIcon]::Information)
    if ($userResponse -ne [System.Windows.Forms.DialogResult]::OK) { return }

    $language = $languages[$cmbLanguage.SelectedItem]
    $installPath = Get-DownloadPath -version $selectedVersion -architecture $architecture -channel $channel -language $language -licenseType $licenseType
    $productId = Get-ProductId -version $selectedVersion -licenseType $licenseType

    # Office 365 offline dosyalarının varlığını kapsamlı kontrol et / Comprehensive check for Office 365 offline files
    $office365RootPath = Join-Path $PSScriptRoot "Office365"
    
    # 1. Office365 klasörünün varlığını kontrol et / Check if Office365 folder exists
    if (-not (Test-Path $office365RootPath)) {
        [System.Windows.Forms.MessageBox]::Show(
            $translations[$script:currentLanguage]['OfflineFilesNotFound'],
            $translations[$script:currentLanguage]['OfflineFilesNotFoundTitle'],
            [System.Windows.Forms.MessageBoxButtons]::OK,
    ...
    # 2. Architecture klasörünün (32 veya 64) varlığını kontrol et / Check if architecture folder (32 or 64) exists
    $architecturePath = Join-Path $office365RootPath $architecture
    ...
    # 3. Language klasörünün varlığını kontrol et / Check if language folder exists
    ...
    # 4. Office\Data klasörünün varlığını kontrol et / Check if Office\Data folder exists
    $officeDataPath = Join-Path $installPath "Office\Data"
    if (-not (Test-Path $officeDataPath)) {
        [System.Windows.Forms.MessageBox]::Show(
            $translations[$script:currentLanguage]['OfflineFilesNotFound'],
            $translations[$script:currentLanguage]['OfflineFilesNotFoundTitle'],
            [System.Windows.Forms.MessageBoxButtons]::OK,
    ...
    # 5. Office\Data klasöründe dosyaların varlığını kontrol et / Check if Office\Data folder contains files
    try {
        $filesInDataDirectory = @(Get-ChildItem -Path $officeDataPath -File -Recurse -ErrorAction Stop)
        if ($filesInDataDirectory.Count -eq 0) {
            [System.Windows.Forms.MessageBox]::Show(
                $translations[$script:currentLanguage]['OfflineFilesNotFound'],
                $translations[$script:currentLanguage]['OfflineFilesNotFoundTitle'],
                [System.Windows.Forms.MessageBoxButtons]::OK,
    ...
    # 6. setup.exe dosyasının root dizinde olup olmadığını kontrol et / Check if setup.exe exists in root directory
    $setupPathRoot = Join-Path $PSScriptRoot "setup.exe"
    if (-not (Test-Path $setupPathRoot)) {
        [System.Windows.Forms.MessageBox]::Show(
            $translations[$script:currentLanguage]['OfflineFilesNotFound'],
            $translations[$script:currentLanguage]['OfflineFilesNotFoundTitle'],
            [System.Windows.Forms.MessageBoxButtons]::OK,
    ...
    try {
        $configPath = Create-ConfigurationXmlOffline -basePath $installPath -productId $productId -architecture $architecture -channel $channel -language $language -excludeApps $excludeApps -additionalProducts $selectedAdditionalProducts

        $statusLabel.Text = $translations[$script:currentLanguage]['Installing']
        # Butonları kilitle
        Lock-Buttons -lock $true
        
        # Önce mevcut animasyonu durdur (varsa)
        Stop-ProgressAnimation
        
        # İlerleme çubuğunu animasyonlu yap (Timer ile manuel Marquee efekti)
        $progressBar.Visible = $true
        $progressBar.Enabled = $true
        ...
        if (-not (Test-Path $setupPath)) {
            Lock-Buttons -lock $false
            throw "$($translations[$script:currentLanguage]['SetupExeNotFound']) $setupPath"
        }

        $process = Start-Process -FilePath $setupPath -ArgumentList "/configure `"$configPath`"" -PassThru -WindowStyle Hidden

        # Kurulum bitene kadar bekle (ilerleme çubuğu Marquee olarak kalacak)
        while (-not $process.HasExited) {
            Start-Sleep -Milliseconds 200
            # Progress bar'ın Marquee animasyonunun çalışması için sürekli güncelle
            if ($progressBar.Style -eq [System.Windows.Forms.ProgressBarStyle]::Marquee) {
                $progressBar.Invalidate()
                $progressBar.Update()
            }
            [System.Windows.Forms.Application]::DoEvents()
        ...
        if ($process.ExitCode -eq 0 -or (Test-OfficeInstalled)) {
            # Office'in düzgün yapılandırılması için kısa bir bekleme ve doğrulama
            $statusLabel.Text = $translations[$script:currentLanguage]['OfficeConfiguring']
            # Animasyonu devam ettir
            if (-not $script:isProgressAnimating) {
                Start-ProgressAnimation
            }
            [System.Windows.Forms.Application]::DoEvents()
            
            # Office'in tamamen yüklenmesi için bekle
            Start-Sleep -Seconds 3
            
            # Office'in düzgün kurulduğundan emin olmak için update/repair çalıştır
            try {
                $verifyProcess = Start-Process -FilePath $setupPath -ArgumentList "/repair" -PassThru -WindowStyle Hidden -ErrorAction SilentlyContinue
                if ($verifyProcess) {
                    $timeout = 30
                    $elapsed = 0
                    while (-not $verifyProcess.HasExited -and $elapsed -lt $timeout) {
                        Start-Sleep -Seconds 1
                        $elapsed++
                        [System.Windows.Forms.Application]::DoEvents()
                    }
                    if (-not $verifyProcess.HasExited) {
                        $verifyProcess.Kill()
                    }
                }
            } catch {
                # Repair başarısız olursa devam et
            }
            
            Start-Sleep -Seconds 1
            Stop-ProgressAnimation
            
            # Company adını registry'ye yaz 
            try {
                # Office ClickToRun Configuration - Ana registry yolu
                $regPathClickToRun = "HKLM:\SOFTWARE\Microsoft\Office\ClickToRun\Configuration"
                try {
                    if (-not (Test-Path $regPathClickToRun)) {
                        New-Item -Path $regPathClickToRun -Force | Out-Null
                    }
                    Set-ItemProperty -Path $regPathClickToRun -Name "Company" -Value "Office 365 Enterprise" -Type String -Force -ErrorAction Stop
                } catch {
                    # ClickToRun yazma hatası görmezden gel (yönetici hakları gerekebilir)
                }
                
                # HKCU registry yolları - Office'in Company bilgisini sakladığı yerler
                $regPaths = @(
                    "HKCU:\Software\Microsoft\Office\Common\UserInfo",
                    "HKCU:\Software\Microsoft\Office\16.0\Common\UserInfo",
                    "HKCU:\Software\Microsoft\Office\15.0\Common\UserInfo"
                )
                foreach ($regPath in $regPaths) {
                    try {
                        ...
                        Set-ItemProperty -Path $regPath -Name "Company" -Value "Office 365 Enterprise" -Type String -Force -ErrorAction Stop
                        Set-ItemProperty -Path $regPath -Name "CompanyName" -Value "Office 365 Enterprise" -Type String -Force -ErrorAction Stop
                    } catch {
                        # Bu registry yolu yazılamazsa devam et
                    }
                }
                
                # HKLM registry yolu (yönetici hakları gerekir) - sistem genelinde
                try {
                    $regPathsLM = @(
                        "HKLM:\SOFTWARE\Microsoft\Office\Common\UserInfo",
                        "HKLM:\SOFTWARE\WOW6432Node\Microsoft\Office\Common\UserInfo"
                    )
                    foreach ($regPathLM in $regPathsLM) {
                        if (-not (Test-Path $regPathLM)) {
                            New-Item -Path $regPathLM -Force | Out-Null
                        }
                        Set-ItemProperty -Path $regPathLM -Name "Company" -Value "Office 365 Enterprise" -Type String -Force -ErrorAction SilentlyContinue
                    }
                } catch {
                    # HKLM yazma hatası görmezden gel (yönetici hakları gerekebilir)
                }
            } catch {
                # Registry yazma hatası görmezden gel
            }
            
            if ($licenseType -eq "Volume") {
                $progressRestarted = $false
                if (-not $script:isProgressAnimating) {
                    Start-ProgressAnimation
                    $progressRestarted = $true
                }
                try {
                    $statusLabel.Text = $translations[$script:currentLanguage]['VolumeLicenseLoading']
                    [System.Windows.Forms.Application]::DoEvents()
                    Install-VolumeLicenseCertificates -architecture $architecture -additionalProducts $selectedAdditionalProducts
                    $statusLabel.Text = $translations[$script:currentLanguage]['VolumeLicenseSuccess']
                    Start-Sleep -Seconds 1
                } catch {
                    $statusLabel.Text = $translations[$script:currentLanguage]['VolumeLicenseError']
                    $statusLabel1.Text = $_.Exception.Message
                } finally {
                    if ($progressRestarted) {
                        Stop-ProgressAnimation
                    }
                }
            }
            ...
            $statusLabel.Text = $translations[$script:currentLanguage]['InstallComplete']
            [System.Windows.Forms.MessageBox]::Show($translations[$script:currentLanguage]['InstallationSuccess'], $translations[$script:currentLanguage]['AboutTitle'], [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information)
        } else {
            $statusLabel.Text = $translations[$script:currentLanguage]['Error']
            Show-DetailedErrorMessage -ExitCode $process.ExitCode
        }
        ...
        # Hata durumunda butonları aç
        Lock-Buttons -lock $false
        Stop-ProgressAnimation
        $statusLabel.Text = $translations[$script:currentLanguage]['Error']
        [System.Windows.Forms.MessageBox]::Show($_.Exception.Message, $translations[$script:currentLanguage]['Error'], [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
    }
})

# ONLINE + DOWNLOAD butonu / ONLINE + DOWNLOAD button
$installButton.Add_Click({
    # İnternet kontrolü
    $internetConnection = Test-Connection -ComputerName "google.com" -Count 1 -Quiet
    if (-not $internetConnection) {
        [System.Windows.Forms.MessageBox]::Show($translations[$script:currentLanguage]['NoInternetConnection'], $translations[$script:currentLanguage]['Error'], [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
        return
    }
    ...
    $userResponse = [System.Windows.Forms.MessageBox]::Show($confirmationMessage, $translations[$script:currentLanguage]['ConfirmDownload'], [System.Windows.Forms.MessageBoxButtons]::OKCancel, [System.Windows.Forms.MessageBoxIcon]::Information)
    if ($userResponse -ne [System.Windows.Forms.DialogResult]::OK) { return }

    $downloadPath = Get-DownloadPath -version $selectedVersion -architecture $architecture -channel $channel -language $language -licenseType $licenseType
    $productId = Get-ProductId -version $selectedVersion -licenseType $licenseType

    try {
        $configPathDownload = Create-ConfigurationXmlDownload -basePath $downloadPath -productId $productId -architecture $architecture -channel $channel -language $language -excludeApps $excludeApps -additionalProducts $selectedAdditionalProducts
        $statusLabel.Text = $translations[$script:currentLanguage]['Downloading']
        ...
        $process = Start-Process -FilePath $setupPath -ArgumentList "/download `"$configPathDownload`"" -PassThru -WindowStyle Hidden

        # İndirme bitene kadar bekle (ilerleme çubuğu Marquee olarak kalacak)
        while (-not $process.HasExited) {
            Start-Sleep -Milliseconds 200
            # Progress bar'ın Marquee animasyonunun çalışması için sürekli güncelle
            if ($progressBar.Style -eq [System.Windows.Forms.ProgressBarStyle]::Marquee) {
        ...
        if ($process.ExitCode -ne 0) {
            Lock-Buttons -lock $false
            Stop-ProgressAnimation
            throw "$($translations[$script:currentLanguage]['DownloadFailed']) $($process.ExitCode)"
        }

        # İlerleme çubuğunu durdur
        Stop-ProgressAnimation
        $statusLabel.Text = $translations[$script:currentLanguage]['DownloadComplete']
        
        # İndirme tamamlandı, butonları geçici olarak aç (kullanıcı onayını bekle)
        Lock-Buttons -lock $false

        if (Test-Path $configPathDownload) { Remove-Item -Path $configPathDownload -Force }

        $installResponse = [System.Windows.Forms.MessageBox]::Show($translations[$script:currentLanguage]['DownloadCompleteQuestion'], $translations[$script:currentLanguage]['ContinueInstallation'], [System.Windows.Forms.MessageBoxButtons]::YesNo, [System.Windows.Forms.MessageBoxIcon]::Question)
        if ($installResponse -ne [System.Windows.Forms.DialogResult]::Yes) { 
            Lock-Buttons -lock $false
            return 
        }

        # OFFLINE mi ONLINE mı?
        $installPath = Get-InstallPath -version $selectedVersion -architecture $architecture -channel $channel -language $language -licenseType $licenseType

        if ($offlineRadioButton.Checked) {
            $configPathInstall = Create-ConfigurationXmlOffline -basePath $installPath -productId $productId -architecture $architecture -channel $channel -language $language -excludeApps $excludeApps -additionalProducts $selectedAdditionalProducts
        } else {
            $configPathInstall = Create-ConfigurationXmlOnline -productId $productId -architecture $architecture -channel $channel -language $language -excludeApps $excludeApps -additionalProducts $selectedAdditionalProducts
        }

        $statusLabel.Text = $translations[$script:currentLanguage]['Installing']
        # Butonları tekrar kilitle
        Lock-Buttons -lock $true
        ...
        $processInstall = Start-Process -FilePath $setupPath -ArgumentList "/configure `"$configPathInstall`"" -PassThru -WindowStyle Hidden

        # Kurulum bitene kadar bekle (ilerleme çubuğu animasyonlu olarak kalacak)
        while (-not $processInstall.HasExited) {
            Start-Sleep -Milliseconds 200
            # Progress bar animasyonu timer ile çalışıyor, sadece DoEvents yeterli
            [System.Windows.Forms.Application]::DoEvents()
        }
        ...
        if ($processInstall.ExitCode -eq 0 -or (Test-OfficeInstalled)) {
            # Office'in düzgün yapılandırılması için kısa bir bekleme ve doğrulama
            $statusLabel.Text = $translations[$script:currentLanguage]['OfficeConfiguring']
            # Animasyonu devam ettir
            if (-not $script:isProgressAnimating) {
            ...
            $statusLabel.Text = $translations[$script:currentLanguage]['Error']
            Show-DetailedErrorMessage -ExitCode $processInstall.ExitCode
        }

        # Butonları tekrar aç
        Lock-Buttons -lock $false
        if (Test-Path $configPathInstall) { Remove-Item -Path $configPathInstall -Force }
...
# LinkLabel'lar / LinkLabels
$LinkLabel1 = New-Object System.Windows.Forms.LinkLabel
$LinkLabel1.Text = $translations[$script:currentLanguage]['MyPage']
$LinkLabel1.Location = New-Object System.Drawing.Point(20, 640)
$LinkLabel1.Size = New-Object System.Drawing.Size(300, 30)
$LinkLabel1.Add_Click({
    Start-Process "https://github.com/abdullah-erturk"
    Start-Process "https://erturk.netlify.app/"
})

$LinkLabel2 = New-Object System.Windows.Forms.LinkLabel
$LinkLabel2.Text = $translations[$script:currentLanguage]['About']
$LinkLabel2.Location = New-Object System.Drawing.Point(730, 640)
$LinkLabel2.Size = New-Object System.Drawing.Size(300, 30)
$LinkLabel2.Add_Click({
    [System.Windows.Forms.MessageBox]::Show($translations[$script:currentLanguage]['AboutMessage'], $translations[$script:currentLanguage]['AboutTitle'], [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information)
})
...
$statusLabel1.Location = New-Object System.Drawing.Point(20, 600)
$statusLabel1.Size = New-Object System.Drawing.Size(800, 400)
$statusLabel1.Name = "statusLabel1"
$statusLabel1.TabIndex = 0
$statusLabel1.Text = ""

function Kontrol-Kosullari {
    # Okunabilir ortam kontrolü - geçici dosya oluşturma/silme yöntemi
    $testFile = Join-Path $PSScriptRoot ".writable_test_$(Get-Random).tmp"
    $isReadOnly = $false
    ...
    try {
        ...
        # Dosya oluşturulamadı veya silinemedi - okunabilir ortam
        $isReadOnly = $true
        # Eğer dosya oluşturulduysa ama silinemediyse, silmeyi tekrar dene
        if (Test-Path $testFile) {
            try {
                Remove-Item -Path $testFile -Force -ErrorAction SilentlyContinue
            } catch {
                # Silinemiyorsa görmezden gel
            }
        }
    }

    if ($isReadOnly) {
        $statusLabel1.Text = $translations[$script:currentLanguage]['ReadOnlyEnvironmentMessage']
        $installOptionsGroup.Enabled = $false
        $installButton.Enabled = $false
        $offlineInstallButton.Enabled = $true
        $cmbLanguage.Enabled = $false
        $onlineRadioButton.Enabled = $false
        $offlineRadioButton.Checked = $true
        $onlineRadioButton.Checked = $false
    } else {
        $statusLabel1.Text = ""
        $installOptionsGroup.Enabled = $true
        $cmbLanguage.Enabled = $true
        $onlineRadioButton.Enabled = $true
        # Radio button durumuna göre butonları ayarla
        if ($offlineRadioButton.Checked) {
            $offlineInstallButton.Enabled = $true
            $installButton.Enabled = $false
        } else {
